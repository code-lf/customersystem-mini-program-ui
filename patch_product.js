const fs = require('fs');
let content = fs.readFileSync('pages/product/index.vue', 'utf-8');
...
if (!content.includes('v-if="isLoading"')) {
  const skeleton = `
      ...
      <template v-else>
  `;
  content = content.replace(/<view class="search-row">/, `${skeleton}\n      <view class="search-row">`);
  content = content.replace(/<view class="tabbar-space" \/>/, `</template>\n      <view class="tabbar-space" />`);
}
fs.writeFileSync('pages/product/index.vue', content);
console.log('patched product/index.vue');
