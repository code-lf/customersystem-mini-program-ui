<template>
  <view class="quote-page">
    ...
    <view v-if="activeTab === 'current' && quoteItems.length && !isLoading" class="quote-footer">
      ...
      <button class="export-main-btn" @click="openExportPanel">导出方案报价</button>
    </view>
...
const normalizeDiscountRate = () => {
  let rate = Math.round(Number(discountRate.value));
...
const exportQuote = async () => {
  ...
  try {
    ...
    try {
      ...
      console.warn('setCartDiscount before export failed:', e);
    }
